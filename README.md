# `БИБЛИОТЕКА ФУНКЦИЙ JAVASCRIPT` </br>(библиотека для "тренировки", некоторый функционал не дружит с webpack)</br>КАК ЗНАК ПРИЗНАНИЯ ХОРОШЕМУ РУКОВОДИТЕЛЮ И ЧЕЛОВЕКУ КРОПОТКИНУ АЛЕКСЕЮ :)
